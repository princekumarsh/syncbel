...
class WebController extends Controller
{
    ...
    public function home()
    {
        ...
        $deal_of_the_day = DealOfTheDay::join('products', 'products.id', '=', 'deal_of_the_days.product_id')->select('deal_of_the_days.*', 'products.unit_price', 'products.thumbnail', 'products.slug','products.name', 'products.details')->where('products.status', 1)->where('deal_of_the_days.status', 1)->first();
        $banner = Banner::where('published','1')->where('banner_type','Main Banner')->get();
    //  dd($deal_of_the_day,$banner);
        return view('web-views.home',
                    compact('featured_products', 'topRated', 'bestSellProduct', 'latest_products', 'categories', 'brands', 'deal_of_the_day', 'top_sellers', 'home_categories', 'brand_setting','banner')
                );
    }
    ...
    public function shoplist()
    {
        ...
        $products = Product::with(['reviews'])->active()->where('status', 1)->OrderBy('id', 'DESC')->get();

        // $featured_products = Product::with(['reviews'])->active()->where('featured_status', 1)->take(10)->get();

        // $random_products = Product::with(['reviews'])->active()->inRandomOrder()->take(8)->get();

        // $latest_products = Product::with(['reviews'])->active()->where('status', 1)->orderBy('id', 'desc')->get();

        // $categories = Category::with(['childes.childes'])->take(12)->get();
        $categories =
        Category::with(['childes.childes'])
        ->where('position', 0)
            ->priority()
            ->get();
        //dd($categories);
        ...
        // $countWishlist = Wishlist::where('product_id', $deal_of_the_day->id)->count();

        return view('web-views.shop', compact('products', 'categories'));

        //  return redirect()->route('shop.apply');
    }
    // {
    ...
    public function singleProduct($id){
        ...
        return view('web-views.single-product');
    }
    ...
    public function checkoutPro(){
        return view('web-views.checkout-product');
    }
    ...
    public function checkout_details(Request $request)
    {
       ...
       // dd( count($cart_group_ids),$countries);
    ...
    public function product($slug)
    {
        ...
        //    dd($product);
            // return view('web-views.single-product', compact('product', 'countWishlist', 'countOrder', 'relatedProducts', 'deal_of_the_day'));
        }
...

    {
        ...
        $categories = Category::with(['childes.childes'])->where([['id','=', 338]])->orWhere([['id','=', 205]])->orWhere([['id','=', 206]])->orWhere([['id','=', 268]])->orWhere([['id','=', 260]])->orWhere([['id','=', 262]])->orWhere([['id','=', 253]])->orWhere([['id','=', 254]])->orWhere([['id','=', 249]])->take(12)->get();
       ...
       return view('web-views.shop_page1', compact('products','categories'));
    ...
    public function storeWishlist(Request $request,$id)
    {
        if ($request->ajax()) {
            if (auth('customer')->check()) {
                $wishlist = Wishlist::where('customer_id', auth('customer')->id())->where('product_id', $id)->first();
                if (empty($wishlist)) {
                    ...
                    $product_count = Wishlist::where(['product_id' => $id])->count();
                    session()->put('wish_list', Wishlist::where('customer_id', auth('customer')->user()->id)->pluck('product_id')->toArray());
                    return response()->json(['success' => $data, 'value' => 1, 'count' => count($countWishlist), 'id' => $id, 'product_count' => $product_count]);
                } else {
    ...
    public function aboutNav(){
        return view('web-views.about-navbar');
    }

    public function contactNav(){
        return view('web-views.contact-navbar');
    }
}