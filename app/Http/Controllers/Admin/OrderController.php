...
class OrderController extends Controller
{
    ...
    public function status(Request $request)
    {
        ...
        foreach ($order->details as $detail) {
            // ShipRocket Api Intergration
                        $orderDetails1 = Order::where('id',$detail['order_id'])->first()->toArray();
                        $other_details = json_decode(json_decode($detail['product_details'])->variation);
                        $product_name = json_decode($detail['product_details'])->name;
                        $customerDetails = User::where('id',$orderDetails1['customer_id'])->first()->toArray();
                        $shippingDetails = ShippingAddress::where('id',$orderDetails1['shipping_address'])->first()->toArray();
                        $orderDetails = array (
                                        'order_id'=> $detail['order_id'],
                                          'order_date'=> $detail['created_at'],
                                          'pickup_location'=> 'Primary',
                                          'channel_id'=> '3732513',
                                          'comment'=> 'Syncbel Technology',
                                          'billing_customer_name'=> $shippingDetails['contact_person_name'],
                                          'billing_last_name'=> '',
                                          ...
                                          'order_items'=> array (
                                                        ...
                                                        array (
                                                          'name'=> $product_name,
                                                          'sku'=> $other_details[0]->sku,
                                                          'units'=> $detail['qty'],
                                                          'selling_price'=> $orderDetails1['order_amount'],
                                                          'discount'=> '',
                                                          'tax'=> '',
                                                          'hsn'=> ''
                                                        ),
                                          ...
                                          'total_discount'=> 0,
                                          'sub_total'=> $orderDetails1['order_amount'],
                                          'length'=> 10,
                                          'breadth'=> 15,
                                          'height'=> 20,
                                          'weight'=> 2.5
                    ...
                    $orderDetails = json_encode($orderDetails);
                        ...
                        curl_setopt_array($curl, array(
                          ...
                          CURLOPT_POSTFIELDS =>'{
                            "email": "customercare@syncbel.com",
                            "password": "Sagarbce@99"
                        }',
